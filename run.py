...
twitch_miner = TwitchChannelPointsMiner(
    username=os.getenv('TwitchLogin'),
    password=os.getenv('TwitchPassword'),           # If no password will be provided, the script will ask interactively
...
twitch_miner.mine(
    [
        Streamer("SametTheTurk", settings=StreamerSettings(make_predictions=False  , follow_raid=True , claim_drops=True  , watch_streak=True , bet=BetSettings(strategy=Strategy.SMART      , percentage=5 , stealth_mode=True,  percentage_gap=20 , max_points=2000   , filter_condition=FilterCondition(by=OutcomeKeys.TOTAL_USERS,      where=Condition.LTE, value=800 ) ) )),
        Streamer("annacramling", settings=StreamerSettings(make_predictions=True , follow_raid=True  , claim_drops=False ,                     bet=BetSettings(strategy=Strategy.SMART , percentage=10 , stealth_mode=False, percentage_gap=20 , max_points=10000  , filter_condition=FilterCondition(by=OutcomeKeys.TOTAL_POINTS,     where=Condition.GTE, value=250 ) ) )),
        Streamer("photochess", settings=StreamerSettings(make_predictions=True  , follow_raid=True ,                     watch_streak=True , bet=BetSettings(strategy=Strategy.SMART      , percentage=5 , stealth_mode=False, percentage_gap=30 , max_points=5000 , filter_condition=FilterCondition(by=OutcomeKeys.ODDS,             where=Condition.LT,  value=300 ) ) )),
